...
describe('Calendar sync (GraphQL)', () => {
  ...
  async function createTask(
    ...
    const dueDateArg = dueDate ? `, dueDate: "${dueDate}T00:00:00.000Z"` : '';
    const body = await graphql<{ createTask: { id: string } }>(
      `mutation { createTask(listId: "${listId}", title: "${title}"${dueDateArg}) { id } }`,
      headers,
    );
  ...
  it('syncs Tasks with a due date and skips Tasks without one', async () => {
    const owner = asUser('owner-1', 'owner@example.com');
    const listId = await createList(owner, 'Groceries');
    await createTask(owner, listId, 'Milk', '2026-09-01');
    await createTask(owner, listId, 'No due date task');
    await connectGoogleCalendar(owner);

    const body = await graphql<{ syncListToCalendar: number }>(
      `mutation { syncListToCalendar(listId: "${listId}") }`,
      owner,
    );

    expect(body.errors).toBeUndefined();
    expect(body.data?.syncListToCalendar).toBe(1);
    expect(createEvent).toHaveBeenCalledTimes(1);
    expect(createEvent).toHaveBeenCalledWith('plain-access-token', {
      summary: 'Milk',
      dueDate: '2026-09-01',
    });
  ...
  it('does not create duplicate events on a re-sync (idempotent)', async () => {
    const owner = asUser('owner-1', 'owner@example.com');
    const listId = await createList(owner, 'Groceries');
    await createTask(owner, listId, 'Milk', '2026-09-01');
    await connectGoogleCalendar(owner);

    const first = await graphql<{ syncListToCalendar: number }>(
      `mutation { syncListToCalendar(listId: "${listId}") }`,
      owner,
    );
    expect(first.data?.syncListToCalendar).toBe(1);

    const second = await graphql<{ syncListToCalendar: number }>(
      `mutation { syncListToCalendar(listId: "${listId}") }`,
      owner,
    );
    expect(second.errors).toBeUndefined();
    expect(second.data?.syncListToCalendar).toBe(0);
    expect(createEvent).toHaveBeenCalledTimes(1);
  ...
  it('only syncs newly-added Tasks on a later sync, leaving already-synced ones alone', async () => {
    const owner = asUser('owner-1', 'owner@example.com');
    const listId = await createList(owner, 'Groceries');
    await createTask(owner, listId, 'Milk', '2026-09-01');
    await connectGoogleCalendar(owner);
    ...
    await createTask(owner, listId, 'Eggs', '2026-09-02');
  ...
  it("an accepted collaborator can sync using their own connection, independently of the owner's", async () => {
    ...
    const ownerSyncs = await testDb.calendarSync.findMany({
      where: { userId: ownerUser.id },
    });
    expect(ownerSyncs).toHaveLength(0);
  });
